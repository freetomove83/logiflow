...
export function ShipperJoinPage() {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    register.mutate({ organizationType: "shipper", organizationName: inviteInfo.data.shipperName, businessNumber, contactName: representative.trim(), loginId: trimmedLoginId, password, inviteToken: token, businessAddress: businessAddress.trim(), contacts: contacts.map(contact => ({ name: contact.name.trim(), department: contact.department.trim(), phone: formatPhone(contact.phone.trim()) })).filter(contact => contact.name.length > 0) });
  };
