...
export function AgencySignupPage() {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (!agencyName.trim() || !manager.trim() || !phone.trim()) return toast.error("대리점명, 담당자명, 연락처를 입력해 주세요."); if (!courier) return toast.error("본사 택배사를 선택해 주세요."); toast.success("다음 단계에서 대표 운영자 아이디와 비밀번호를 설정해 주세요."); setLocation(`/account-setup?role=agency&organization=${encodeURIComponent(agencyName)}&courier=${encodeURIComponent(courier)}`); };
  return <OnboardingShell step="01" title="대리점 워크스페이스 개설" subtitle="대표 대리점 정보와 초대 담당자를 등록하세요. 이후 초대 링크와 화주 협업 공간이 생성됩니다."><form onSubmit={submit} className="onboard-form"><label>대리점명 <Input value={agencyName} onChange={event => setAgencyName(event.target.value)} placeholder="예: 서울중앙물류" /></label><label>사업자 등록번호 <Input placeholder="예: 123-45-67890" inputMode="numeric" /></label><label>본사 택배사 <select value={courier} onChange={event => setCourier(event.target.value)}><option value="">택배사 선택</option>{COURIERS.map(item => <option key={item} value={item}>{item}</option>)}</select></label><div className="form-grid"><label>대표 담당자명 <Input value={manager} onChange={event => setManager(event.target.value)} placeholder="예: 김대리" /></label><label>담당자 연락처 <Input value={phone} onChange={event => setPhone(formatPhone(event.target.value))} placeholder="예: 010-1234-5678" inputMode="tel" /></label></div><label>업무용 이메일 <Input placeholder="예: operation@agency.co.kr" type="email" /></label><div className="consent-box"><CheckCircle2 /><p><strong>대리점 운영자 권한</strong><span>화주 초대, 담당자 배정, 증빙 검토, 공지 메시지 발송 준비 권한이 부여됩니다.</span></p></div><Button type="submit" className="onboard-submit">워크스페이스 개설하고 화주 초대하기 <ArrowRight /></Button></form></OnboardingShell>;
}
...
export function ShipperJoinPage() {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!/^\d{10}$/.test(businessNumber)) return toast.error("사업자등록번호 10자리를 숫자로 입력해 주세요.");
    if (businessAddress.trim().length < 5) return toast.error("사업장 주소를 입력해 주세요.");
    if (productCategory.trim().length < 1) return toast.error("주력 상품 카테고리를 입력해 주세요.");
    ...
    register.mutate({ organizationType: "shipper", organizationName: inviteInfo.data.shipperName, businessNumber, contactName: representative.trim(), loginId: trimmedLoginId, password, inviteToken: token, businessAddress: businessAddress.trim(), productCategory: productCategory.trim(), contacts: contacts.map(contact => ({ name: contact.name.trim(), department: contact.department.trim(), phone: formatPhone(contact.phone.trim()) })).filter(contact => contact.name.length > 0) });
  };
