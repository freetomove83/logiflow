...
export function AgencyInvitePage() {
  ...
  const generate = () => {
    if (!/^[0-9A-Za-z-]{6,24}$/.test(shipperContractNumber.trim())) return toast.error("계약 택배 번호(송장번호) 6~24자를 입력해 주세요.");
...
export function ShipperJoinPage() {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    register.mutate({ organizationType: "shipper", organizationName: inviteInfo.data.shipperName, businessNumber, contactName: representative.trim(), loginId: trimmedLoginId, password, inviteToken: token, contacts: contacts.map(contact => ({ name: contact.name.trim(), department: contact.department.trim(), phone: contact.phone.trim() })).filter(contact => contact.name.length > 0) });
  };
